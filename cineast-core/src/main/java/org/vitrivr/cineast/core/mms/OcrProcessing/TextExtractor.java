...
public class TextExtractor {
    ...
    public List<Rect> getTextRects(Mat src){
        List<Rect> texts = new ArrayList<Rect>();

        Mat grad = new Mat();
        Mat morphKernel = Imgproc.getStructuringElement(Imgproc.MORPH_ELLIPSE, new Size(3,3));

        Imgproc.morphologyEx(src, grad, Imgproc.MORPH_GRADIENT , morphKernel);
        Mat bw = new Mat();
        Imgproc.threshold(grad, bw, 0.0, 255.0, Imgproc.THRESH_BINARY | Imgproc.THRESH_OTSU);
        ...
        for(int idx = 0; idx < contours.size(); idx++)
        {
            ...
            if (r > .45 && (rect.height > 8 && rect.width > 8))
                rectangle(src, rect.br() , new Point( rect.br().x-rect.width ,rect.br().y-rect.height),  new Scalar(0, 255, 0));
